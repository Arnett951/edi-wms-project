    ...
    <>
      <section className="panel">
        ...
        <p className="admin-lede">
          Requests generated by the intake pipeline (<code>pipeline/generate_change_request.py</code>) move
          through three gates before landing on <code>main</code>. Gate 1 (SuperUser or Admin can Approve or
          Reject) only marks a CR ready -- it never starts a build. Once a CR reaches Pending Build Approval,
          only Admin can actually Start Build (with an optional budget override) or Reject it there. Tier C
          requests are auto-denied by the pipeline itself and never reach Gate 1 -- they're listed here for
          visibility only. Implemented CRs show a merge-readiness badge -- a red "needs attention" flag means
          the branch has conflicts and won't be a clean one-click merge. Merge and Rollback on the deployed
          site run asynchronously via GitHub Actions, so a dispatched action shows a status badge until it
          actually lands rather than a button you can immediately re-click. Use the tabs above to switch
          between in-flight and Closed/Merged CRs.
        </p>
